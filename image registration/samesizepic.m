...
%I am trying to run the algorithm for same size pictures. supposed the
%target is the orginal pic rotate 90 degree and cut into same size picture
%with the orginal pic
...
y = [(regimage(s+1/(bm-tm+1),t,r)-regimage(s,t,r))/(1/(bm-tm+1))
     (regimage(s,t+1/(bn-tn+1),r)-regimage(s,t,r))/(1/(bn-tn+1))
...
Z = imrotate(B,r*180,'bilinear');
%Z1 = imtranslate(Z,[-s*(bm-tm),-t*(bn-tn)]);
Z1 = imtranslate(Z,[-s*(bm-tm+1),-t*(bn-tn+1)]);
Z2 = Z1(1:tm,1:tn);
